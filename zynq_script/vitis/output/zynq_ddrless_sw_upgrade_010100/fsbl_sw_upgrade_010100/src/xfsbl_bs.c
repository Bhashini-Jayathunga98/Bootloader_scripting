...
int ConfigureBitstream(XFsblPs *FsblInstancePtr, u32 Address, u32 bs_size)
{
	u32 Status = XFSBL_SUCCESS;
	u32 NumChunks = 0U;
	u32 RemainingBytes = 0U;
	...
	//StartAddrByte = ImageOffset + 4*(PartitionHeader->DataWordOffset);
	StartAddrByte = Address;

	XFsbl_Printf(DEBUG_INFO,
			"Nonsecure Bitstream to be copied from %0x. Size is %d bytes \r\n",
			StartAddrByte, bs_size);

	BitStreamSizeByte = bs_size;
	...
	for(Index = 0; Index < NumChunks; Index++)
	{
		...
		//Change byte order on buffer
		for (int i=0; i<READ_BUFFER_SIZE; i=i+4) {
			temp = *(ReadBuffer+i);
			*(ReadBuffer+i) = *(ReadBuffer+i+3);
			*(ReadBuffer+i+3) = temp;
			temp = *(ReadBuffer+i+1);
	...
	if(RemainingBytes != 0U)
	{
		...
		//Change byte order on buffer
		for (int i=0; i<RemainingBytes; i=i+4) {
			temp = *(ReadBuffer+i);
			*(ReadBuffer+i) = *(ReadBuffer+i+3);
			*(ReadBuffer+i+3) = temp;
			temp = *(ReadBuffer+i+1);
	...
	if (Status != XFSBL_SUCCESS) {
		XFsbl_Printf(DEBUG_PRINT_ALWAYS, "PL wait for done failed!\n\r");
		goto END;
	}
